...
maius = 'ABOCDEFGHIJKLMNOPQRST'
def troca(x):
    resp = ""
    for c in x:
        resp = resp + maius[ordem.index(c)]
        #index dá a posição de c na string ordem
    return resp
txtA.sort(key=troca)
print(txtA)
...
def googlon(x):
    ...
    for d in x:
        ...
        p20 = p20*20
    return soma
print (googlon("zmbzmb"))

#set significa conjinto

for x in txtA:
    if googlon(x)%42 == 0 and len(x) == len (set(x)):
        print (x)
